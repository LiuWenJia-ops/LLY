...
void ReplaceWindow::showFindText()
{
    ...
    if(!hasResult){
       ...
       //------------------有结果---------------------
        // for( nowResult=1;resultPtr->getSpecificOne(nowResult)[0]<=mwPtr->textBody.getRow()&&resultPtr->getSpecificOne(nowResult)[2]<mwPtr->textBody.getCol();nowResult<=number){
        // }//最后一个或者后面一个
