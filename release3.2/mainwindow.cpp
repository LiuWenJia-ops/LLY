...
Dialog::Dialog(QWidget *parent) : 
    QDialog(parent)
{
    setWindowTitle(QStringLiteral("File Dialog"));
    fileBtn =new QPushButton;
    fileBtn->setText(tr("Please input file path"));
...
void Dialog::showFile()
{
    ...
    QString qstr = fileLineEdit->text();    

    if(!mwPtr->addr.empty()){//已经打开过一份
        QMessageBox::StandardButton rb = QMessageBox::question(this, "Open file", 
            ...
            { 
                    mwPtr->on_actionsave_triggered();
                    mwPtr->addr.clear();//清空地址
            }
    }//清空body，还原cur位置

    mwPtr->textBody.delFULL();
    mwPtr->textEdit->clear();
    
     QByteArray ba= qstr.toUtf8();
    mwPtr->addr=ba.data();//用户选择的文件所在位置
    std::ifstream buf(mwPtr->addr);
    if(buf.fail()){//打开失败
        int failure = QMessageBox::warning(this, "Fail to open file!",
         ...
         mwPtr->textBody.delFULL();
         mwPtr->flush();
         mwPtr->textBody.setAxis(1,1);
         if(failure==QMessageBox::Ok){
             this->hide();
         }
         //地址还是保存
        return;
    ...
    while (!buf.eof()){//没到文件尾
        getline(buf,media);
        qstr = QString(media.c_str());
        mwPtr->textEdit->append(qstr);//显示到textedit
        whl.append(media);whl.append("\n");
    }
    mwPtr->textBody.insertStr(whl);
    mwPtr->textBody.printFULL();
    mwPtr->tcursor=mwPtr->textEdit->textCursor();//设置光标
    buf.close();
}
//----------------------------------------主窗口----------------------------------------------
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent)
{
    ...
    connect(uploadAction,&QAction::triggered,this,&MainWindow::on_actionupload_triggered);
    //定义下载
    ...
    connect(downloadAction,&QAction::triggered,this,&MainWindow:: on_actiondownload_triggered);
    //-----------------
    //定义findAction
    ...
    //-----------------------------------------创建排版控制工具栏---------------------------
     ...
     //自动换行
    WraptooBar = new QToolButton;
     WraptooBar->setIcon (QIcon(":/wrap.png"));
     WraptooBar->setStatusTip("Wrap");
     WraptooBar->setCheckable (true);
     ...
     FonttoolBar->addWidget (WraptooBar);
     ...
     //连接槽函数
      ...
      connect (WraptooBar, SIGNAL(clicked(bool)), this, SLOT(ShowWraptooBar()));
    //---------------------------------------------状态栏----------------------------------------
...
{  
    QMessageBox::StandardButton rb = QMessageBox::question(this, " Quit",
                                                            "Do you want to save the changes of the current text?",
                                                             QMessageBox::Yes | QMessageBox::No, 
                                                             QMessageBox::Yes); 
    if(rb == QMessageBox::Yes)//保存文本框内容到地址
...
void MainWindow::ShowWraptooBar(){
    if(WraptooBar->isChecked())
        textEdit->setLineWrapMode(QTextEdit::WidgetWidth); 
    else
        textEdit->setLineWrapMode(QTextEdit::NoWrap);
    if(WraptooBar->isChecked())
       WraptooBar->setChecked(true);
    else
        WraptooBar->setChecked(false);
}
...
void MainWindow::on_actionsave_triggered()
{
    ...
    if(addr.empty()){//新建的文件无原地址，打开系统文件夹
        ...
        QMessageBox::StandardButton rb = QMessageBox::question(this, "Save file", 
                                                "Do you want to save the current text in the same path and same name?",
                                                    QMessageBox::Yes | QMessageBox::No, 
                                                    QMessageBox::Yes); 
        if(rb == QMessageBox::No)//保存文本框内容到地址
        { 
            QString filepath = QFileDialog::getSaveFileName(//得到文件路径
                                    this,
                                    tr("Save file"),
...
void MainWindow::upSyncFile(void)
{
    QString toSyncFileAddress;
    toSyncFileAddress=QString("d:/tempSync.txt");
    //=====save first
    ...
    buf.close();
    putServerFile(toSyncFileAddress,"synctest3.txt");
}
void MainWindow::downSyncFile(void)
{
    QString toSyncFileAddress;
    toSyncFileAddress=QString("d:/tempSync.txt");

    std::fstream cleanfile;
    cleanfile.open(toSyncFileAddress.toStdString(),std::ios::trunc);
    cleanfile.close();

    getServerFile(toSyncFileAddress,"synctest3.txt");
    ...
    QString qsStr;
    QString filename=toSyncFileAddress;
    if (!filename.isNull()) { //用户选择了文件
        QByteArray ba=filename.toUtf8();
        std::string Aaddr=ba.data();//用户选择的文件所在位置
        std::fstream buf(Aaddr);
        if(buf.fail()){//打开失败
            int failure = QMessageBox::warning(this, "Fail to open file!!!",
                                                            "Please choose right format",
                                                                QMessageBox::Ok,
...
//        tcursor=textEdit->textCursor();//设置光标
        ...
        QFile * readText=new QFile(toSyncFileAddress);
        readText->open(QIODevice::ReadWrite);
